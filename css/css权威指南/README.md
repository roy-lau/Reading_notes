...
* 十六进制RGB颜色
	...
	其工作原理：将三个介于00~FF的十六进制数连起来,就可以设置一种颜色。这种记法一般语法是 #RRGGBB。 注意，在这三个之间没有逗号，空格或其它分隔符。
...
这个例子定义了一个所谓的 **绝对URL(absolute URL)** 。这里的绝对是指，不论这个URL放置在哪（或许放置在上层页面），它都能正常工作，因为它定义了web空间的中的一个绝对位置。假设你有一个名为`www.baidu.com` 的服务器，该服务器下有一个pix的目录，在这个目录下有个图像`waffle22.gif`,这种情况下，该图像的绝对URL是：
...
单位和值的覆盖面很广，从长度单位到描述效果（如：underline）的特殊关键字，再到颜色单位，还包括文件(如图像)位置。大多数情况下，在单位方面，用户代理几乎能做到完全正确，不过也存在少量的bug和奇怪的问题来烦你。例如，`navigator 4.x` 不能正确的解释相对URL，这些被创作人员过分夸大，以至于过分的依赖绝对URL，用户代理在颜色领域也几乎完全胜任，但同样不乏一些小问题。不过由于存在太多不同的长度单位，尽管这绝对不是bug，但实际上这才是创作人员需要解决的一个有意思的问题。

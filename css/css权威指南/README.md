...
相对长度之所以得名,是因为他们是根据其他事物的关系来度量的。所度量的实际（或绝对）距离可能因为不在其控制之下的其他因素而改变,如屏幕分辨率、可是区域的宽度。用户首选项的设置,以及很多其他方面。另外对于某些相对单位,其大小总是对应于使用该单位的元素,因此会因元素的不同而不同。

共有3种相对长度单位：em、ex和px。前两个单位代表“em-height”和“x-height”,这是常用的印刷度量单位；不过，在CSS中他们有另外的含义,最后一种长度单位是px,这代表像素。如果仔细查看屏幕，一个像素就是计算机显示器上查看到的一个点.这个值被定义为相对单位,因为它取决于显示器的分辨率。
...
一般来讲，如果你做了如下声明：`font-size: 18px;`,web浏览器几乎总会在显示器上使用实际像素——毕竟，显示器上已有像素——不过，如果采用其他显示设备（如打印机），用户代理就必须将像素长度重新缩放为更合理的度量。换句话说打印机代码必须确定一个像素有多少个点，为此，他可能要使用96ppi做为参考像素。

_由于可能会进行重新缩放，像素被定义为一种相对度量单位,尽管在web设计中像素表现的很像绝对单位_

* 如何取舍

根据上述所有问题，最好的度量可能是相对单位,特别是`em`,如果合适也可以使用`px`。在大多数当前浏览器中,由于`ex`实际上就是`em`的一部分(0.5em),所以对目前来说`ex`还不太有用，如果更多的用户代理支持实际的x高度度量,`ex`可能就会有用武之地了。总的来说`em`是最为灵活的，因为他随字体的大小缩放,所以元素和元素操作都能更为一致。

对于元素的其他方面,可能更适合使用像素,如果元素的边框或定位。这完全由具体情况而定.

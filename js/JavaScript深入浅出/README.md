...
> bug在不同的浏览器里会有不同的表现方式

未定义： 指未(使用var)创建的变量，或虽然创建了，却未制定值的变量。无论是哪种意义，问题都在于引用了没有值得变量。

`alert('It's so exciting!')`

在字符串里使用引号与撇号字符，而不是把它们解释为字符串边界，这是个常见的错误。所以我们刚才看到的`alert`框代码具有语法错误，javaScript解释器无法分辨出那个撇号是字符串边界，那个又该当成撇号字符串本身。幸好。有个声明字符串是个真正字符的便利方式——**转义符(escape character)**，加上反斜线(\\)从而把它后面的字符实际视为字符原本的意义。

`alert('It\'s so exciting!')`

__转义符用在字符串中表示实际字符。__

**语法错误(syntax error)：** 由于违反javaScript语言规则而造成的错误，表示程序代码不适合于javaScript解释器中运行。

**逻辑错误(logic error)：** 也就是合乎javaScript语法规则，但完全不符合程序意图的错误。

**运行时错误(runtime error)：** 运行时错误只在脚本运行时发生特性情况时出现。

语法错误、逻辑错误、运行时错误、是网页中常见的三种错误！

常见`bug`清单:

- 确定括号成对出现

- 确定围起代码块的大括号成对出现——小心的缩排代码有助于维持成对的大括号

- 努力避免打错标识符名称——变量与函数的名称如果不一致，很容易造成大问题。

- 在使用引号与撇号时，维持使用方式的一致、如果需要，在HTML属性中小心地混用这两个符号。

- 使用转义符，为字符串中具有特殊意义的字符编码，例如引号(\")或者撇号(\')。

- 千万绝对切莫在想用`==`时误用了`=`。javaScript或许不会把这点视为错误，但你的程序代码运作得讲跟你想的不一样。

- 确定对象在被访问前已经创建 —— 主要发生在网页元素上，网页元素要到`onload`实价触发的前一刻才完成创建。

- 别为局部变量和全局变量取相同的名称，因为局部变量将掩蔽全局变量，造成某些无法预测的行为。

### 12、 动态数据

Ajax让网页能动态接收网络服务器的数据

__XML是种为任何类型的数据设计格式的标记语言__

在`HTML`里的"ML",是指`markup language`(标记语言)，也指出了HTML使用`标签(tag)`和`(attribute)`创建超文本(HT)的事实。正如HTML这种标记语言用于创建超文本网页，XML标记语言用于创建，几乎一切你想创建的东西。这就是"X"的意思——任何事件！既然有这各种能因存储成标签与属性而获益的数据，何不扩展标记语言的触角，以解决其他数据问题？

__XHTML是遵守XML的较严格语法规则的新版HTML： XML+HTML=XHTML__

扩展名或许不同，但XML与HTML有着非常重要的联系，就是XHTML。XHTML是现代版的HTML，遵循XML的严格规则。例如：XHTML网页中的每个起始标签必须有相应的结尾标签。HTML的语法比较快速但松散，有时候就算没为`<p>`加上`</p>`，还是可以过关。XHTML就没这么容忍，他要求这类标签必须成对出现。

__只有XML还不是动态的，但它刚好与Ajax和DOM都能呢紧密结合__

XML是最常与Ajax合用的数据格式，因此也是表示博客数据的合理侯选者。数据驱动的博客将在服务器端与客户端间频繁往来传送。XML的高度结构化本质，让它成为快速运送数据的理想选择。

而且`XML`与`HTML(XHTML)`相似，遂可透过DOM，把XML视为节点树而访问数据。也就是说，可以设计javaScript代码，在XML节点树中往复移动，小心的分离出想要的数据，然后动态地融合数据于网页。这么多优点，使XML是个创建动态，数据驱动网页时的良好数据存储方案。

__XMLHttpRequest__

javaScript内置一个称为`XMLHTTPRequest`的对象，用于发起Ajax请求处理ajax响应。这个对象非常复杂，包含许多通力合作以支持Ajax的方法与特性。

- **readyState** 请求的状态代码：0(未初始)、1(开启)、2(已传送)、3(接收中)、4(已载入)。
...
开启请求时，必须制定请求的类型(GET或POST)，还有服务器的URL，以及请求是否"异步"。**非同步请求(asynchronous request)**在后台运作，不用让脚本等待，所以几乎所有Ajax请求都是异步请求。

Ajax请求的**类型(type)**非常重要，不只反映传送到服务器的事物，也反映请求的**意图**。请求类型，或称请求方法(method)之一，是`GET`，主要用于从服务器取得数据，而不影响服务器上的其他东西。另一种请求类型， `POST` 通常与传送数据到服务器有关，传送后的服务器状态通常有所改变，以响应传入的数据。

- **GET** 不会改变服务器上任何事物的数据获取方式。如果需要，还能透过URL传入小量数据给服务器。GET非常适合从服务器上的XML文件获取博客数据。

- **POST** 此处传送给服务器的数据，因为某种方式会改变服务器状态，例如存储数据到数据库中。数据还是能在响应中返回。POST 是使用网页表单动态新增博客日志这类任务的理想帮手。

__处理响应__

- **getResponseText()** 以纯文本的格式获取Ajax响应里的访问。
